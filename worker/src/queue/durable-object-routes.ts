			...
			const response = await handleQueueAction(
				{
					...
					getJob: context.getJob,
					listMissionJobs: context.listMissionJobs,
					reconcileJob: context.reconcileJob,
					persistJob: context.persistJob,
					getMission: context.getMission,
					reconcileMission: context.reconcileMission,
					persistMission: context.persistMission,
					listMissions: context.listMissions,
					writeAudit: context.writeAudit,
					buildJobAudit: context.buildJobAudit,
					buildJobProgressSnapshot: context.buildJobProgressSnapshot,
					buildMissionProgressSnapshot: context.buildMissionProgressSnapshot,
