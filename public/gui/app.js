(function () {
	...
	function deriveJobId(payload) {
		...
		if (payload.kind === 'opengpt.notification_contract.job_progress') return payload.progress && payload.progress.job_id ? String(payload.progress.job_id) : '';
		if (payload.kind === 'opengpt.notification_contract.jobs_list') {
			if (state.selectedJobId) return state.selectedJobId;
			return Array.isArray(payload.jobs) && payload.jobs[0] && payload.jobs[0].job_id ? String(payload.jobs[0].job_id) : '';
		}
		...
		if (payload.kind === 'opengpt.notification_contract.permission_bundle') return payload.notification && payload.notification.job_id ? String(payload.notification.job_id) : '';
		if (payload.kind === 'opengpt.notification_contract.incident_bundle') return Array.isArray(payload.runs) && payload.runs[0] && payload.runs[0].job_id ? String(payload.runs[0].job_id) : '';
		return '';
	}
	...
	function runSummary(payload) {
		...
		if (payload.kind === 'opengpt.notification_contract.incident_bundle') {
			...
			const run = runs.find(function (item) {
				return item.job_id === state.selectedJobId;
			}) || runs[0];
	...
	function extractStructured(result) {
		...
		if (hasRecord(result.structuredContent)) return result.structuredContent;
		return null;
	}

	function persist() {
		const currentBridge = bridge();
		if (!currentBridge || typeof currentBridge.setWidgetState !== 'function') return;
		try {
			currentBridge.setWidgetState({
				tab: state.tab,
		...
		return `<section class="panel full-span">
			<div class="tab-header">
				...
				<form class="filter-row" data-form="feed-filters">
					...
					<button class="mini-button" type="submit"${bridge() && currentJobId ? '' : ' disabled'}>Reload</button>
				</form>
			</div>
		...
		root.innerHTML = `<div class="app-shell ${escapeHtml(config.mode)}">
			<header class="topbar">
				...
				<div class="topbar-meta">
					<div class="topbar-card"><span>Mode</span><strong>${escapeHtml(bridge() ? 'ChatGPT widget' : 'Standalone preview')}</strong></div>
			...
			${!bridge() ? `<section class="panel compact-panel standalone-panel"><p class="panel-kicker">Standalone preview</p><h3>Live bridge not attached</h3><p class="supporting-copy">This route renders demo data locally. Open the same widget from ChatGPT to enable direct MCP tool calls.</p></section>` : ''}
			...
			<main class="content-shell">
				...
				<div class="split-grid">
					<section class="panel info-panel">
						...
						<div class="detail-list"><div><span>Blocked action</span><strong>${escapeHtml(blocker && blocker.blocked_action ? blocker.blocked_action : 'n/a')}</strong></div></div>
					</section>
					<section class="panel info-panel">
				...
				<section class="panel action-panel">
					...
					<div class="action-row">
						<button class="action-button secondary" type="button" data-action="load-jobs"${bridge() ? '' : ' disabled'}>Load jobs</button>
						<button class="action-button" type="button" data-action="refresh-current"${bridge() && currentJobId ? '' : ' disabled'}>Refresh run</button>
						<button class="action-button" type="button" data-action="load-feed"${bridge() && currentJobId ? '' : ' disabled'}>Load event feed</button>
					</div>
					<p class="supporting-copy">${bridge() ? 'This widget can call MCP tools directly through the ChatGPT bridge.' : 'Standalone preview mode only shows demo data.'}</p>
				</section>
				<section class="panel">
	...
	async function runTool(name, args, nextTab) {
		if (!bridge() || typeof bridge().callTool !== 'function') {
			state.error = 'Live MCP calls are only available inside the ChatGPT widget bridge.';
			render();
			return;
		}
		state.error = '';
	...
	function hydrate() {
		const currentBridge = bridge();
		if (currentBridge && hasRecord(currentBridge.widgetState)) {
			state.tab = typeof currentBridge.widgetState.tab === 'string' ? currentBridge.widgetState.tab : state.tab;
			state.selectedJobId =
				typeof currentBridge.widgetState.selectedJobId === 'string' ? currentBridge.widgetState.selectedJobId : '';
		}
		if (currentBridge && hasRecord(currentBridge.toolOutput)) {
			state.payload = currentBridge.toolOutput;
			state.toolInput = hasRecord(currentBridge.toolInput) ? currentBridge.toolInput : {};
			state.message = 'Widget bridge connected.';
			if (typeof currentBridge.setOpenInAppUrl === 'function') {
				try {
					currentBridge.setOpenInAppUrl({ href: `${config.appOrigin}/gui/` });
				} catch (error) {
					console.warn(error);
				}
			}
