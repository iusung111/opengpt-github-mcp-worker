...
export function buildMcpServer(env: AppEnv): McpServer {
	...
	server.registerTool(
		...
		async ({ query }) => {
			try {
				return toolText(ok(buildHelpPayload(query), readAnnotations));
			} catch (error) {
				return toolText(fail(errorCodeFor(error, 'help_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		{
			description:
				'Request a bundle of permissions for a repository to streamline the workflow. This tool is a signal to the user to approve a set of actions at once.',
			inputSchema: {
				repos: z.array(z.string()).describe('List of owner/repo'),
				capabilities: z.array(z.string()).describe('e.g. read, write, workflow, pr'),
				reason: z.string().describe('Why are these permissions needed?'),
			},
			annotations: readAnnotations,
		},
		async ({ repos, capabilities, reason }) => {
			try {
				diagnosticLog('permission_bundle_request', { repos, capabilities, reason });
				return toolText(
					ok(
						{
							status: 'acknowledged',
							message: 'Permissions bundle acknowledged. You may proceed with the requested actions.',
							repos,
							capabilities,
						},
						readAnnotations,
				...
				return toolText(fail(errorCodeFor(error, 'request_permission_bundle_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		async ({ owner, repo, include_completed_jobs }) => {
			...
			try {
				...
				return toolText(fail(errorCodeFor(error, 'repo_work_context_failed'), error, readAnnotations));
			} finally {
	...
	server.registerTool(
		...
		async ({ repo_key }) => {
			try {
				...
				return toolText(fail(errorCodeFor(error, 'workspace_activate_failed'), error, writeAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		async ({ repo_key, preferred_root }) => {
			try {
				...
				return toolText(fail(errorCodeFor(error, 'workspace_resolve_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		async ({ repo_key, workspace_path, display_name, aliases }) => {
			try {
				...
				return queueActionResult(result, writeAnnotations);
			} catch (error) {
				return toolText(fail('queue_action_failed', error, writeAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		async ({ query, repo_key }) => {
			try {
				...
				return toolText(fail(errorCodeFor(error, 'workspace_find_similar_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		async () => {
			try {
				...
				return toolText(fail(errorCodeFor(error, 'workspace_list_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		{
			description:
				'Inspect the GitHub self-repo plus configured Cloudflare live and mirror health endpoints for maintenance and self-improvement checks.',
			inputSchema: {
		...
		async ({ include_healthz }) => {
			try {
					...
					ok(
						{
							...
							recent_self_deploy_runs: (workflowRunsResult.workflow_runs ?? [])
				...
				return toolText(fail(errorCodeFor(error, 'self_host_status_failed'), error, readAnnotations));
			}
		},
	);
	...
	server.registerTool(
		...
		{
			description:
				'Deploy the self repo through the self-deploy workflow with mirror-first guardrails. Use mirror for self-improvement validation, then explicitly promote to live.',
			inputSchema: {
				deploy_target: z.enum(['mirror', 'live']).default(getSelfDefaultDeployTarget(env)),
				reason: z.string().optional(),
				expected_commit_sha: z.string().optional(),
				verify_mirror_first: z.boolean().default(true),
			},
			annotations: writeAnnotations,
		},
		async ({ deploy_target, reason, expected_commit_sha, verify_mirror_first }) => {
			try {
				const selfRepoKey = getSelfRepoKey(env);
				const [owner, repo] = selfRepoKey.split('/');
				const distinctMirror = mirrorConfigured(env);
				if (deploy_target === 'live' && selfRequiresMirrorForLive(env) && !distinctMirror) {
					throw new Error('live self deploy requires a distinct mirror url to be configured first');
				}
				if (deploy_target === 'live' && verify_mirror_first) {
					const mirrorHealth = await fetchHealthSnapshot(getSelfMirrorUrl(env));
					if (!mirrorHealth?.ok) {
						throw new Error('mirror health check failed; refusing live promotion');
					}
				}
				...
				if (expected_commit_sha) {
					inputs.expected_commit_sha = expected_commit_sha;
				}
				await githubPost(env, `/repos/${owner}/${repo}/actions/workflows/${workflowId}/dispatches`, {
					ref: getDefaultBaseBranch(env),
					inputs,
				});
				return toolText(
					ok(
						{
							...
							verify_mirror_first,
							reason: reason ?? null,
							expected_commit_sha: expected_commit_sha ?? null,
						},
						writeAnnotations,
					),
				);
